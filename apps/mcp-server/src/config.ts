// The MCP server authenticates as a specific Ledgeur user so every query
// runs under that user's RLS — external tools can never see meetings the user
// isn't allowed to see. Preferred: LEDGEUR_REFRESH_TOKEN (long-lived, auto-
// refreshing). Also supports a short-lived LEDGEUR_ACCESS_TOKEN.
...
export async function getClientFromEnv(): Promise<SupabaseClient> {
  ...
  if (!refreshToken && !accessToken) {
    throw new Error(
      "Set LEDGEUR_REFRESH_TOKEN (recommended) or LEDGEUR_ACCESS_TOKEN — generate it in the app " +
