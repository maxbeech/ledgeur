...
clusters, a "Merge into…" recovery) only ever landed in `packages/core`'s
`cluster.ts` — the web/browser diarization path. The shipped desktop app does
not use that code at all: it calls sherpa-onnx's own fast clustering directly
from Rust (`engine.rs`), which has no equivalent of the tiny-cluster
correction, so every noisy or interrupted turn that didn't fall inside
`DIARIZE_DISTANCE_THRESHOLD` of an existing voice kept becoming a permanent
new "speaker" — unbounded, since pyannote's segmentation model routinely cuts
far more turns than there are people in the room. This is very likely also
why "Suggest names" reported finding nothing on a real recording: the
transcript the model was shown was fragmented across dozens of phantom
speaker labels instead of the handful of real ones, which is not a
transcript a 1.5B model can find an introduction in reliably.

`fold_tiny_clusters` (new, `src-tauri/src/ai/mod.rs`) ports the same
correction to the native pipeline: after sherpa's pass, any diarized cluster
under `MIN_SPEAKER_MS` (2s, matching `MIN_SPEAKER_SECONDS` on the web side)
is folded into whichever surviving cluster its own audio sounds most similar
to, however weakly, using a new `engine::cluster_embeddings` (one CAM++
embedding per cluster, reusing the clip-collection `identify_speakers`
already did for voice-profile matching). Pure and unit-tested, including the
reported shape of the bug directly: 42 clusters — 2 real speakers plus 40
sub-second slivers — fold back down to the 2 real ones. The existing
"Merge into…" action in `SpeakerPanel` remains the by-hand recovery for
whatever still gets through.

## Unreleased (2026-09-22) — Meeting notes stayed a handful of bullets no matter how much was actually discussed

The bullet summary is deliberately terse by design — one line per point, for
skimming — but there was nothing else on offer for someone who wanted the
full detail of what was actually said: the context, the reasoning, the
numbers, who argued what. `MeetingNotes` gained an optional `detailedNotes`
field: a full narrative write-up in Markdown, generated by a second,
independent on-device model pass (`generateDetailedNotes`, `notes.ts`) with
its own prompt (`DETAILED_SYSTEM`) asking for a `##` heading per topic with
the discussion underneath, not condensed to its conclusion. It gets its own
reply budget (1536 tokens against the bullet pass's 768) and its own,
smaller transcript window (`DETAILED_CHUNK_CHARS`) so the larger reply
doesn't silently crowd the transcript out of the model's 8192-token context.
A long meeting is windowed the same way the bullet notes are, but the
windows are concatenated under "Part N" headings rather than reduced — a
reduce pass would compress exactly the detail this exists to keep. Only
attempted once the bullet pass has already proven the model can answer at
all, and never allowed to fail the meeting: a write-up that couldn't be
written just leaves the field unset. Rendered in `MeetingDetail` via a small
Markdown-subset reader (`parseNotesMarkdown`, new, `packages/core` — headings,
paragraphs and bullet lists only, which is all the prompt ever produces) and
folded into `notesToMarkdown`'s `## Meeting notes` section, so it travels
with the stored record and the Notion export for free. Local to the
recording device for now — not yet part of the cloud sync payload.
